...
suppressWarnings(config <- read.table(config,
                                      sep = ",",
                                      stringsAsFactors = F,
                                      header = T)
                 )
...
get_hgncs <- function(fmodelid, all_models) {
  ...
  if (all(is.null(unlist(fmodel$references)))) {
    return(NULL)
    }
  ...
  ret <- list(
    list(name = model_name,
         hgncs = unlist(sapply(fmodel$references[fmodel$type %in% c("Protein",
                                                                    "RNA",
                                                                    "Gene")],
                               function(x) x[x$type == "HGNC_SYMBOL",
                                             "resource"]))))
  ...
  if (length(unique(fmodel[fmodel$type == "Pathway", "name"])) > 0) {
    ...
    for (pname in unique(me[fmodel$type == "Pathway", "name"])) {
      res <- apply(me[fmodel$name == pname, "bounds"], 1,
                   function(x) fmodel$bounds$x >= x["x"] &
                     fmodel$bounds$x <= (x["x"] +  x["width"]) &
                     fmodel$bounds$y >= x["y"] &
                     fmodel$bounds$y <= (x["y"] +  x["height"]))

      hgncs <- unlist(sapply(fmodel$references[res & fmodel$type %in%
                                                 c("Protein", "RNA", "Gene")],
                             function(x) x[x$type == "HGNC_SYMBOL",
                                           "resource"]))
